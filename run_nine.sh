...
echo "Running TReX"
python run_trex_curr.py --num-seeds "$seeds" --src-env "$srcenv" --tgt-env "$room" --transfer "$tr" --discount-r "$dr" --discount-kd "$dkd" --env-name "$room" --num-iters "$iters" --epsilon "$eps" --temp "$temperature" --explore "$exp" --lfp-iters "$lfit"