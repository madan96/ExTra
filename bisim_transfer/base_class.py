...
class Bisimulation(object):
    def __init__(self, opts):
        ...
        self.src_agent = QLearningAgent(alpha, epsilon, discount, 4, self.src_env.state_space, self.src_env.tp_matrix, self.src_env.blocked_positions, 712)
        self.tgt_agent = QLearningAgent(alpha, epsilon, discount, 4, self.tgt_env.state_space, self.tgt_env.tp_matrix, self.tgt_env.blocked_positions, 712)
        self.transferred_agent = QLearningAgent(alpha, epsilon, discount, 4, self.tgt_env.state_space, self.tgt_env.tp_matrix, self.tgt_env.blocked_positions, 712)
        ...
        for s1_pos, s1_state in self.src_env.state2idx.items():
            ...
            for s2_pos, s2_state in self.tgt_env.state2idx.items():
                ...
                for a in range(self.action_space):
                    next_state, reward_a, done, next_possible_states = self.src_env.stepd(a)
                    self.src_env.start_position = s1_pos
                    self.src_env.position = s1_pos
                    for b in range(self.action_space):
                        next_state, reward_b, done, next_possible_states = self.tgt_env.stepd(b)
