...
from qlearn import train_trail, test
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        if (os.path.isfile('transfer_logs/Dist-sa_' + args.src_env + '_' + args.tgt_env + '.npy')
            ...
            bisimulation.tgt_env.save_transfer_image(bisimulation.transferred_agent.qvalues, os.path.join('transfer_logs', args.tgt_env + '-' + str(args.discount_r) + '-' + str(args.discount_kd) + '.png'))
        ...
        train_trail.train(bisimulation, args)
    else:
        test.test(args)
    end = time.time()
